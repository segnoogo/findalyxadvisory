...
function rpAssertionBox(sl,txt,x,y,w,h){
  x=(x==null?0.55:x);y=(y==null?1.36:y);w=(w==null?12.23:w);h=(h==null?0.44:h);
  sl.addShape("roundRect",{x:x,y:y,w:w,h:h,rectRadius:0.02,fill:{color:"F7F9FC"},
    line:{color:RP.NAVY,width:1,dashType:"dash"}});
  sl.addText(txt,{x:x+0.18,y:y+0.03,w:w-0.36,h:h-0.06,fontSize:10.5,italic:true,
    color:RP.G_TITRE,fontFace:"Arial",valign:"middle"});
}
...
function construireDD(pptx){
  ...
  rpTable(sl,0.55,2.15,6.55,B.societe.toUpperCase()+" - Compte de résultat",pl.entetes,pl.lignes,pl.styles,
    pl.colsDelta,pl.largeurs,8,"Source : balances générales "+fy[0]+" - "+fy[fy.length-1]);
  rpColonnes(sl,7.4,2.15,5.4,2.6,"("+rpLib()+")",fy,[
    {name:"CA",values:A.map(a=>v.CA[a]),color:"172554"},
    {name:"EBITDA",values:A.map(a=>v.EBITDA[a]),color:"2E5AAC"},
    {name:"Résultat net",values:A.map(a=>v.RESULTAT_NET[a]),color:"8FAADC"}]);
  ...
  rpTable(sl,0.55,2.15,7.0,B.societe.toUpperCase()+" - Actif net",bs.entetes,bs.lignes,bs.styles,
  ...
  rpBarreStructure(sl,7.85,2.35,4.95,"Structure de l'actif "+fy[fy.length-1],[
  ...
  rpCadreComment(sl,7.85,3.75,4.95,3.1);
  rpPied(sl,mention,++page);
  /* BFR et délais */
  sl=pptx.addSlide();
  ...
  if(A.length>1){
    ...
    rpTable(sl,0.55,2.15,6.9,B.societe.toUpperCase()+" - TFT",
    ...
    rpCadreComment(sl,7.6,2.15,5.2,4.7);
    rpPied(sl,mention,++page);
  }
...
function construireValo(pptx){
  ...
  rpBarresH(sl,7.65,3.35,5.15,2.2,"Fourchette — fonds propres ("+rpLib()+")",["Bas","DCF","Multiples","Haut"],
    [val.eqMin,val.equityDcf,val.equityMult,val.eqMax],{colors:["9CA3AF","172554","2E5AAC","9CA3AF"]});
  ...
  rpTable(sl,0.55,3.6,12.25,"Projections sous-jacentes "+ap[0]+" - "+ap[ap.length-1],
    ...
    [3.2,...Array(ap.length).fill(1.2)],9);
  rpColonnes(sl,0.55,5.1,7.4,1.75,"FCFF actualisés ("+rpLib()+")",fyp,[{name:"FCFF actualisés",values:ap.map(a=>val.pv[a]),color:"16904E"}]);
  rpCadreComment(sl,8.15,5.1,4.65,1.75);
  rpPied(sl,mention,++page);
