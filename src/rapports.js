...
function rpPreambule(pptx,B,mention,page){
  const sl=pptx.addSlide();rpEnTete(sl,B.societe,"Préambule");rpTitre(sl,"Préambule et périmètre d'intervention");
  const fy=B.fy;
  const blocs=[
    ["Contexte de la mission","Le présent rapport restitue les constats issus de la revue financière de "+B.societe+" sur les exercices "+fy[0]+" à "+fy[fy.length-1]+". Il constitue un support de discussion préalable aux échanges avec le management, et n'a pas vocation à être diffusé hors de ce cadre."],
    ["Base de travail","Les analyses sont établies à partir des balances générales importées, retraitées selon le référentiel SYSCOHADA. Les agrégats (marge brute, EBITDA, BFR, flux de trésorerie, ratios) sont reconstitués automatiquement et réconciliés avec le bilan."],
...
function rpSlideRatios(pptx,B,mention,page){
  ...
  rpAssertion(sl,R.synthese+"  (score de santé financière : "+R.score+"/100)");
  ...
  rpObservations(sl,9.35,2.0,3.45,[
    ["Répartition",R.nbGood+" indicateur(s) solide(s), "+R.nbWarn+" à surveiller, "+R.nbBad+" fragile(s) sur "+(R.nbGood+R.nbWarn+R.nbBad)+" évalués."],
    ["Lecture","Les seuils sont indicatifs et doivent être relativisés selon le secteur d'activité."],
    ["Benchmark","Une comparaison sectorielle est disponible dans l'application (module Analyse financière)."]]);
  rpPied(sl,mention,page);
}
...
function construireDD(pptx){
  ...
  rpObservations(sl,7.4,5.2,5.4,[
    ["Chiffre d'affaires","Le CA passe de "+rpFmt(v.CA[a0])+" ("+fy[0]+") à "+rpFmt(ca1)+" ("+fy[fy.length-1]+")"+(tcam!==null?", TCAM "+rpPct(tcam):"")+"."],
    ["Coûts","Frais généraux "+rpFmt(-v.OPEX[a1])+" "+rpLib()+" ("+rpPctCA(-v.OPEX[a1],ca1)+" du CA) ; personnel "+rpFmt(-v.CHARGES_PERSONNEL[a1])+" "+rpLib()+"."]]);
  rpPied(sl,mention,++page);
  ...
  rpObservations(sl,0.55,5.9,12.25,[
    ["Structure de coûts","Les coûts directs représentent "+rpPctCA(-v.COUTS_DIRECTS[a1],ca1)+" du CA et les frais généraux "+rpPctCA(-v.OPEX[a1],ca1)+" ; les charges de personnel s'élèvent à "+rpFmt(-v.CHARGES_PERSONNEL[a1])+" "+rpLib()+", soit "+rpPctCA(-v.CHARGES_PERSONNEL[a1],ca1)+" du CA."]]);
  rpPied(sl,mention,++page);
  /* situation nette */
  sl=pptx.addSlide();
  rpEnTete(sl,B.societe,"Due diligence financière");
  ...
  rpObservations(sl,7.85,5.35,4.95,[
    ["BFR","Le BFR ressort à "+rpFmt(v.BFR[a1])+" "+rpLib()+", soit "+(ca1?Math.round(v.BFR[a1]*360/ca1):0)+" jours de CA."],
    ["Capitaux propres",v.CAPITAUX_PROPRES[a1]<0?"Capitaux propres négatifs : situation à redresser.":"Structure à apprécier au regard de l'endettement financier."]]);
  rpPied(sl,mention,++page);
  /* BFR et délais */
  sl=pptx.addSlide();
  rpEnTete(sl,B.societe,"Analyse financière");
  ...
  rpGraphBarres(sl,0.55,3.35,6.1,3.35,"BFR en jours de CA",fy,[{name:"BFR (jours)",values:A.map(a=>bfrJ(a)),color:"172554"}],{fmt:"#,##0"});
  rpGraphBarres(sl,7.0,3.35,5.8,3.35,"BFR en valeur ("+rpLib()+")",fy,[{name:"BFR",values:A.map(a=>rq(v.BFR[a])),color:"224289"}],{fmt:"#,##0"});
  rpPied(sl,mention,++page);
  /* cash flows */
  if(A.length>1){
    sl=pptx.addSlide();
    ...
    rpObservations(sl,7.6,5.5,5.2,[
      ["Flux opérationnels","La récurrence de la CAFG et la contribution du BFR doivent être analysées sur la période."],
      ["Investissements","Les investissements reconstitués sont à rapprocher du plan d'investissement de la société."]]);
    rpPied(sl,mention,++page);
  }
