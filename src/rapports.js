...
function construireTeaser(pptx){
  ...
  sl.addText(nom,{x:0.55,y:0.6,w:9.4,h:0.55,fontSize:27,bold:true,color:RP.BLANC,fontFace:RP_TITRE});
  ...
  /* amorce rédigée (même fabrique que le mémorandum) puis premier fragment de description */
  const PRt=rpProse(B,I);
  const desc=rpCoupe(PRt.lead+(PRt.puces.length?(" "+PRt.puces[0]):""),430);
