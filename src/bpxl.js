...
function construireFeuillesBP(wb){
  ...
  Object.assign(hy,{dso:hb,dio:hb+1,dpo:hb+2,tva:hb+3,tvaEx:hb+4,ac:hb+5,ad:hb+6,dfs:hb+7,capex:hb+8,amort:hb+9,
    tauxEx:hb+10,durEx:hb+11,empr:hb+12,tauxN:hb+13,durN:hb+14,pfin:hb+15,payout:hb+16,
    rf:hb+18,pm:hb+19,beta:hb+20,ppays:hb+21,ptaille:hb+22,pilliq:hb+23,kd:hb+24,wd:hb+25,g:hb+26,mc:hb+27,mt:hb+28,adjE:hb+29,
    exitM:hb+30,pdcf:hb+31,pcomp:hb+32,ptrans:hb+33,panr:hb+34,
    seuil:hb+35,repdef:hb+36,decouvert:hb+37});
  ...
  hRow(hy.tva,"TVA — taux de droit commun");hVal(hy.tva,(H.tva!=null?+H.tva:0.18),PCT2);
  hRow(hy.tvaEx,"Chiffre d'affaires exonéré de TVA (1 = oui, 0 = assujetti)");hVal(hy.tvaEx,(H.tvaExonere?1:0),"0");
  ...
  const defsB=[
   ...
   {rn:rb.CLI,lib:"Créances clients",hist:a=>v.CLIENTS[a],
    f:i=>`${P(rp.CA,i)}*(1+IF(${h1(hy.tvaEx)}=1,0,${h1(hy.tva)}))*${h1(hy.dso)}/360`},
  ...
  [-0.01,-0.005,0,0.005,0.01].forEach((dg,jx)=>{
    const c=cellF(wsV,rs,3+jx,`"g "&ROUND((${G}+${dg})*100,1)&" %"`,undefined);
    c.font={bold:true,color:{argb:"FF172554"}};c.alignment={horizontal:"right"};
  });
  [-0.01,-0.005,0,0.005,0.01].forEach((dw,ix)=>{
    const rn=rs+1+ix;
    cellF(wsV,rn,2,`"WACC "&ROUND((${W}+${dw})*100,2)&" %"`).font={bold:true,color:{argb:"FF172554"}};
    [-0.01,-0.005,0,0.005,0.01].forEach((dg,jx)=>{
      const w=`(${W}+${dw})`,g=`(${G}+${dg})`;
      cellF(wsV,rn,3+jx,
