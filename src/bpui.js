...
function mTableVNC(M){
  ...
  if(!S.length)return '';
  var brut=[],cum=[],vnc=[]; for(i=0;i<N;i++){brut.push(0);cum.push(0);vnc.push(0);}
  var body=S.map(function(s){
    for(var k=0;k<N;k++){ brut[k]+=s.brut[k]; cum[k]+=s.cum[k]; vnc[k]+=s.vnc[k]; }
    return '<tr><td class="l gc1"><div class="ghead" style="padding-left:3px">'+esc(s.name||'Poste')+'</div></td>'
      +(G_FX?'<td class="l gc2"><span class="gfx"><span class="gchip">'+gN(s.montant)+'</span> sur '+s.duree+' ans'+(s.mes>s.annee?'<span class="mut"> · en service an '+s.mes+'</span>':'')+'</span></td>':'')
      +s.vnc.map(function(v){return '<td><span class="gcell gcalc num"'+(v<=0.01?' style="color:var(--muted)"':'')+'>'+(v>0.01?gN(v):'&ndash;')+'</span></td>';}).join('')+'</tr>';
  }).join('');
  body+='<tr class="gres"><td class="l gc1"><div class="gdrv"><span class="gop">+</span> Valeur brute</div></td>'
    +(G_FX?'<td class="gc2"></td>':'')+brut.map(function(v){return '<td><span class="gcell gcalc num">'+gN(v)+'</span></td>';}).join('')+'</tr>'
    +'<tr class="gres"><td class="l gc1"><div class="gdrv"><span class="gop">&minus;</span> Amortissements cumulés</div></td>'
    +(G_FX?'<td class="gc2"></td>':'')+cum.map(function(v){return '<td><span class="gcell gcalc num" style="color:#c0392b">'+gN(v)+'</span></td>';}).join('')+'</tr>'
    +'<tr class="gtot"><td class="l gc1">Valeur nette comptable (VNC)</td>'+(G_FX?'<td class="gc2"></td>':'')
    +vnc.map(function(v){return '<td class="num v">'+gN(v)+'</td>';}).join('')+'</tr>';
  return '<div class="gbar" style="margin-top:18px"><span class="gt">Immobilisations — valeur nette comptable</span><span class="mut">en FCFA</span></div>'
    +'<div class="gwrap"><div class="gscroll"><table class="gtab"><thead>'+gHead(N,nc,'Poste','Base · durée')+'</thead><tbody>'+body+'</tbody></table></div>'
    +'<div class="gfoot"><span>VNC = valeur brute &minus; amortissements cumulés. C\'est la base du <b>coût du capital</b> (VNC × WACC) et de l\'actif net. '
    +'Les <b>intérêts de construction</b> capitalisés, s\'il y en a, s\'ajoutent aux immobilisations du bilan et ne figurent pas dans ce tableau des postes saisis.</span></div></div>';
}
function mTableCapex(M){
  var N=M.nb||5, nc=(M.dureeConstruction||0), CX=(M.capex||[]), S=gCapexSeries(M), i, tot=[], amo=[];
  ...
  CX.forEach(function(c,ci){
    var s=S[ci], cells='';
    for(var k=0;k<N;k++){
      var py=k+1, v=(py===s.annee)?s.montant:0; tot[k]+=v; amo[k]+=s.dot[k];
      cells+=(py===s.annee)
    ...
    body+='<tr><td class="l gc1"><div class="ghead"><input class="gnm big" value="'+esc(c.name||'')+'" onchange="mCapex('+ci+',\'name\',this.value)">'
      ...
      +cells+'</tr>'
      /* amortissement PROPRE à ce poste, sous sa ligne */
      +'<tr><td class="l gc1"><div class="gdrv" style="padding-left:14px"><span class="gop eq">=</span> <span class="mut">dotation</span></div></td>'
      +(G_FX?'<td class="l gc2"><span class="gfx" style="color:var(--muted)">'+gN(s.montant)+' &divide; '+s.duree+' ans'+(s.mes>s.annee?' · dès l\'an '+s.mes:'')+'</span></td>':'')
      +s.dot.map(function(v){return '<td><span class="gcell gcalc num"'+(v<=0.01?' style="color:var(--muted)"':'')+'>'+(v>0.01?gN(v):'&ndash;')+'</span></td>';}).join('')+'</tr>';
  });
  body+='<tr class="gres"><td class="l gc1">Total investi</td>'+(G_FX?'<td class="gc2"></td>':'')
    +tot.map(function(v){return '<td><span class="gcell gcalc num">'+(v?gN(v):'&ndash;')+'</span></td>';}).join('')+'</tr>'
    +'<tr class="gtot"><td class="l gc1">Dotation totale aux amortissements</td>'
    +(G_FX?'<td class="l gc2" style="font-size:11px;color:var(--muted)">somme des dotations ci-dessus</td>':'')
    +amo.map(function(v){return '<td class="num v">'+(v?gN(v):'&ndash;')+'</td>';}).join('')+'</tr>';
...
function vueModele(){
  ...
  if(SOUS_MODELE==="rev"){
    ...
    corps=mTableCapex(M)+mTableVNC(M);
