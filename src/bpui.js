...
function vueBP(){
  ...
  const tabs=[["hyp","Hypothèses"],["pl","P&L prévisionnel"],["bs","Bilan prévisionnel"],["tft","Flux de trésorerie"],["dette","Dette"]]
  ...
  <div class="row" style="margin:14px 0 12px">${tabs}</div>${corps}`;
}
...
function vueValo(){
  ...
  const bridge=`<div class="deux"><div class="card">
    ...
    ${[["Somme des FCFF actualisés",V.sommePv],["Valeur terminale (g = "+pc(V.g)+")",V.vt],
      ...
      `<tr class="${i===3||i===5?"total":""}"><td>${l}</td><td class="num">${fmt(x)}</td></tr>`).join("")}
    </table></div>
...
function dessinerFootball(){
  ...
  const etiquettesFF={id:"etiquettesFF",afterDatasetsDraw(chart){
    ...
    meta.data.forEach((barre,i)=>{
      const m=V.methodes[i];
      const y=barre.y;
      ctx.fillStyle="#172554";
      ctx.textAlign="right";
      ctx.fillText(fmtFF(m.min*u.f),barre.base-6,y+4);
      ctx.textAlign="left";
      ctx.fillText(fmtFF(m.max*u.f),barre.x+6,y+4);
      /* central au-dessus du marqueur orange */
      const xC=chart.scales.x.getPixelForValue(m.central*u.f);
      ctx.fillStyle="#FA6706";
      ctx.textAlign="center";
      ctx.fillText(fmtFF(m.central*u.f),xC,y-barre.height/2-5);
    });
    ctx.restore();
  }};
