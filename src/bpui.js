...
function vueModele(){
  ...
  if(SOUS_MODELE==="rev"){
    ...
    corps='<div class="card"><div class="sec-titre" style="margin-top:0">Coûts directs</div><div class="mut">Réglés par ligne (onglet Revenus) : % du CA ou coût unitaire × volume. Inflation partagée des coûts unitaires ci-dessous.</div>'
      +'<div class="hyp-l" style="margin-top:10px"><span>Inflation des coûts unitaires</span><input class="sel" style="width:46%" value="'+((M.inflation||0)*100)+'" onchange="mSet(\'inflation\',(numFR(this.value)||0)/100)"> %</div></div>';
    ...
    corps=mVueResultats(P,u);
  }
  return '<h1>Business plan — modèle <span class="chip" style="background:#fff4e8;color:#b45608">sans balance</span></h1>'
    +'<div class="mut" style="margin-bottom:12px">Le compte de résultat est construit à partir d\'inducteurs, puis le moteur enchaîne bilan bouclé, trésorerie et valorisation — comme un dossier avec historique.</div>'
    +kpis
    +'<div class="row" style="margin:14px 0 12px;flex-wrap:wrap">'+barre+'</div>'
    +corps;
}
