...
var WIZ=null;
function ouvrirWizard(){WIZ={step:1,nom:"",secteur:"Général",infos:{},showInfos:false,dossierId:null};VUE="wizard";shell();}
function wizStepper(){
...
function wizSetSecteur(i){WIZ.secteur=SECTEURS[i]||"Général";if(WIZ.dossierId&&DOSSIER){DOSSIER.secteur=WIZ.secteur;sauverDossier();}rendre();}
...
function wizEtapeBalances(){
  const bal=(DOSSIER?DOSSIER.balances:[]).slice().sort((a,b)=>a.annee-b.annee).map(b=>{
...
function wizEtapeRecap(){
  const f=(DOSSIER&&DOSSIER.infos)||{},nb=DOSSIER?DOSSIER.balances.length:0;
  const bal=DOSSIER?DOSSIER.balances.slice().sort((a,b)=>a.annee-b.annee).map(b=>{
  ...
  return `<b>Récapitulatif</b>
   <div class="wiz-recap" style="margin-top:8px">
     <div><span class="mut">Société</span><b>${esc(DOSSIER?DOSSIER.societe:"")}</b></div>
     <div><span class="mut">Secteur (benchmark)</span><b>${esc(DOSSIER?DOSSIER.secteur:"")}</b></div>
     ${info}
     <div><span class="mut">Balances</span><span>${bal||'<span class="chip bad">aucune</span>'}</span></div>
   </div>
   <div class="mut" style="margin-top:12px">${nb?"Tout est prêt — lancez l’analyse.":"Revenez à l’étape « Balances » pour importer au moins un exercice."}</div>`;
}
function vueWizard(){
  if(!WIZ)return '<div class="mut">…</div>';
  const s=WIZ.step;
  const corps=s===1?wizEtapeSecteur():s===2?wizEtapeSociete():s===3?wizEtapeBalances():wizEtapeRecap();
  const gauche=s>1?`<button class="btn" onclick="wizPrecedent()">← Précédent</button>`:`<button class="btn" onclick="wizAnnuler()">Annuler</button>`;
...
async function wizSuivant(){
  wizLire();
  if(WIZ.step===2){
    if(!WIZ.nom){toast("Entrez le nom de la société");return;}
    if(!WIZ.dossierId){
      const id="d"+Date.now();
      ...
      DOSSIER={id,societe:WIZ.nom,secteur:WIZ.secteur,infos:WIZ.infos,balances:[],overrides:{}};
      WIZ.dossierId=id;localStorage.setItem(ACTIF_KEY,id);sauverDossier();
    }else{DOSSIER.societe=WIZ.nom;DOSSIER.secteur=WIZ.secteur;DOSSIER.infos=WIZ.infos;sauverDossier();}
  }
  WIZ.step=Math.min(4,WIZ.step+1);shell();
}
function wizPrecedent(){wizLire();WIZ.step=Math.max(1,WIZ.step-1);shell();}
function wizAnnuler(){const had=WIZ&&WIZ.dossierId;WIZ=null;VUE=had?"import":"accueil";shell();}
function wizLancer(){
  if(!DOSSIER||!DOSSIER.balances.length){toast("Importez au moins une balance pour lancer l’analyse");return;}
  recalculer();WIZ=null;VUE="analyse";shell();
}
...
function importerBalance(){
  ...
  try{
    ...
    DOSSIER.balances.push({annee,fichier:fichier.name,feuille,comptes:res.comptes,controle:res.controle});
